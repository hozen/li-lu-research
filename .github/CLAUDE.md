...
**一切以 `git checkout origin/dev -- <文件>` 拉取的最新内容为准。**
...
dev = api("/branches/dev")
print("Remote dev SHA:", dev["commit"]["sha"])
print("Remote dev date:", dev["commit"]["commit"]["message"])
```
...
```
第一步：获取远程 dev 最新状态
  → 用 GitHub API 获取 origin/dev 最新 SHA
  → 确认你要基于哪个 commit 工作

第二步：读取目标文件的实际内容
  → git checkout origin/dev -- <文件>（或 GitHub API 获取 blob）
  → 验证任务描述的问题是否真实存在
